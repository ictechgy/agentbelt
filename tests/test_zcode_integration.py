...
def prepare(home,env):
    ...
    env.update({'ZCODE_HOME':str(home/'.zcode'),'AGENT_GUARD_BACKEND':'zcode-v1','AGENT_GUARD_BOOTSTRAP':'zcode'})
reads=['/Applications/ZCode.app',g.ROOT/'agent_guard.py',g.ROOT/'zcode_hook.py',g.ROOT/'riskgate_bridge.py',g.ROOT/'vendor',g.ROOT/'state/zcode-agent-config.json']
...
reads=[g.ROOT/'agent_guard.py',g.ROOT/'zcode_hook.py',g.ROOT/'riskgate_bridge.py',g.ROOT/'vendor']
sys.exit(g.run_confined('riskgate-engine-test',Path(sys.argv[2]),['/Library/Developer/CommandLineTools/usr/bin/python3','-I','-c',inner,str(g.ROOT),sys.argv[3]],extra_reads=reads,extra_env={'AGENT_GUARD_BACKEND':'zcode-v1'},ephemeral=True))
"""
