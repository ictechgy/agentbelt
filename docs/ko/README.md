...
| 터미널 | 전용 PTY와 필터링된 stdout/stderr. 입력 주입(`TIOCSTI`) 거부. |
...
Google Chrome과 `agent-browser`가 설치돼 있으면 Kimi/OpenCode 보호 세션은
`shots/<name>.html`을 렌더해 같은 이름의 PNG를 돌려줍니다. CSS·폰트·이미지는
프로젝트 안에 두세요. 비밀 경로·링크·디렉터리 목록은 파일 서버가 거부하고,
브라우저는 전용 프로필·드라이버 세션·외부로 전달하지 않는 로컬 프록시를 사용합니다.
Chrome의 기본 샌드박스는 유지합니다. 큐 쓰기는 고정한 디렉터리 서술자를 사용해
경로가 교체돼도 호스트 파일로 출력이 넘어가지 않게 합니다.
...
복제본은 `3.14.3`로 버전을 고정하고 bundle ID
...
검토한 3.14.3 배포본에서는 기존 저장소 스냅샷 수집 경로가 제거됐으며 전체 ASAR와
CLI 해시로 해당 버전을 고정합니다. 남아 있는 대화 공유 업로드는 로컬 패치로 차단합니다.
DB 준비에는 번들 저장소 전용 Worker를 사용하고 모델 세션은 보호 백엔드를 거칩니다.
...
- **주입된 GitHub 토큰은 세션에 보입니다.** 토큰은 환경 변수가 아니라 격리 홈의 파일(`gh`용 `~/.config/gh/hosts.yml`, git용 `.git-credentials`)로만 전달합니다. 같은 사용자의 프로세스는 다른 프로세스의 환경 변수를 읽을 수 있기 때문입니다. 모델이 직접 보진 않지만 에이전트가 그 파일을 출력하면 컨텍스트에 실립니다. 에이전트가 push 해야 하는 프로젝트에만 주입하세요.
