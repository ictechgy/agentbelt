...
`agent_guard`. Nothing in this package is read or executed by the confined child, with one exception:
`restore_history.py` re-executes itself inside a sandbox and therefore adds its own path to the read
allowance. The core (`agent_guard.py`, `sandbox_runner.mjs`, `environment_notice.py`, `zcode_hook.py`,
